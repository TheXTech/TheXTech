    ...
    for(int A : NPCQueues::Active.may_erase)
    {
        ...
        if(NPC[A].JustActivated != 0)
        {
               ...
               NPC[A].Type != NPCID_FALL_BLOCK_BROWN && !NPCIsACoin[NPC[A].Type]) // And .Type <> NPCID_SPIKY_THROWER
            {
                ...
                bool shared_screen = (g_compatibility.multiplayer_pause_controls) && (activ_screen.Type == ScreenTypes::Dynamic) && (activ_screen.DType == DScreenTypes::Shared);

                int activ_player = (shared_screen) ? 0 : NPC[A].JustActivated;
