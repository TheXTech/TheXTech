...
void RenderGL::loadTextureInternal(StdPicture &target, uint32_t width, uint32_t height, uint8_t *RGBApixels, uint32_t pitch, bool is_mask, uint32_t least_width, uint32_t least_height)
{
    ...
    const GLenum cpu_format = (m_forceFormat > 0) ? m_forceFormat : GL_RGBA;
#else
    target.d.format = (m_forceFormat > 0) ? m_forceFormat :
    ...
    auto internal_format = (m_forceFormat > 0 || m_gl_profile == SDL_GL_CONTEXT_PROFILE_ES) ? target.d.format : GL_RGBA;
