...
int PlayerBox::Logic()
{
    ...
    if(m_just_added)
    {
        ...
        if((int)Controls::g_InputMethods.size() > s_minPlayers && (c_Back || SharedControls.MenuBack))
            m_input_ready = false;
        // if pressing back, don't play drop item noise
        else if(c_Back || SharedControls.MenuBack)
            play_noise = false;
    ...
    else if(c_Back || (Is1P() && SharedControls.MenuBack))
    {
        if(Back())
            return -1;
    }
    else if(c_Do || c.Start || (Is1P() && SharedControls.MenuDo))
    {
        if(Do())
        {
