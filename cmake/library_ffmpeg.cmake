...
if(USE_SYSTEM_LIBS)
    ...
    elseif(APPLE)
        ...
        set(FFMPEG_ARCH_ARGS ${FFMPEG_ARCH_ARGS} --target-os=darwin --toolchain=hardened --enable-cross-compile)

        if("${TARGET_PROCESSOR}" STREQUAL "x86_64")
            set(FFMPEG_ARCH_ARGS ${FFMPEG_ARCH_ARGS} --arch=x86_64)
        elseif("${TARGET_PROCESSOR}" STREQUAL "i386")
            set(FFMPEG_ARCH_ARGS ${FFMPEG_ARCH_ARGS} --arch=i386)
        elseif("${TARGET_PROCESSOR}" STREQUAL "arm" OR "${TARGET_PROCESSOR}" STREQUAL "armv7" OR "${TARGET_PROCESSOR}" STREQUAL "armv7s")
            set(FFMPEG_ARCH_ARGS ${FFMPEG_ARCH_ARGS} --arch=arm)
        elseif("${TARGET_PROCESSOR}" STREQUAL "arm64")
            set(FFMPEG_ARCH_ARGS ${FFMPEG_ARCH_ARGS} --arch=aarch64)
        elseif("${TARGET_PROCESSOR}" STREQUAL "ppc")
            set(FFMPEG_ARCH_ARGS ${FFMPEG_ARCH_ARGS} --arch=ppc)
        elseif("${TARGET_PROCESSOR}" STREQUAL "ppc64")
            set(FFMPEG_ARCH_ARGS ${FFMPEG_ARCH_ARGS} --arch=ppc64)
        endif()
    else()
        set(FFMPEG_BASH_RUNTIME)
        set(FFMPEG_MAKE_TOOL make)
